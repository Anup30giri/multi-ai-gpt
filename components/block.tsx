...
export function Block({
  ...
  const {
    ...
    block && block.status !== 'streaming' ? `/api/document?id=${block.documentId}` : null,
    fetcher
  );
  ...
  return (
    ...
    >
      {!isMobile && (
        <motion.div
          className='relative basis-1/2 bg-muted dark:bg-background h-dvh shrink-0'
      ...
      <motion.div
        className='fixed md:relative  dark:bg-muted bg-background h-dvh basis-1/2 flex flex-col shadow-xl overflow-y-auto'
        initial={
          isMobile
            ? {
                opacity: 0,
